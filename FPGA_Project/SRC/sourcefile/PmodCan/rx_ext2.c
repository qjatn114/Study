...
/* RX */
		if((ReceiveStatus & MCP_STAT_RX0IF) == MCP_STAT_RX0IF)
		{
			printf("Receive SUCCESS RX0IF= %d\n",ReceiveStatus);
			readMsgBuf(fd2, &rx_id, &rx_len, rxBuf, MCP_STAT_RX0IF);
		...
		else if( (ReceiveStatus & MCP_STAT_RX1IF) == MCP_STAT_RX1IF)
		{
			printf("Receive SUCCESS RX1IF=%d\n",ReceiveStatus);
			readMsgBuf(fd2, &rx_id, &rx_len, rxBuf, MCP_STAT_RX1IF);
