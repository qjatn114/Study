...
/* Rx Func */
uint8_t readMsgBuf(int fd, uint16_t *id, uint8_t *len, unsigned char *rxBuf);
uint8_t readMsg(int fd);
...
*/
	...
	for(;;)
	{
		ReceiveStatus = mcp25625_readStatus(fd2);	
		ReceiveStatus &= MCP_STAT_RXIF_MASK;

		Receive = (ReceiveStatus & MCP_STAT_RX0IF)|(ReceiveStatus & MCP_STAT_RX1IF);
...
/* RX */
		...
		{
			printf("Receive = %d\n",Receive);
			readMsgBuf(fd2,&rx_id,&rx_len,rxBuf);
	
			if( (myCanMsg.canIDE & 0x80000000) == 0x80000000 )
			{
				printf("Extended ID : 0x%x  DLC : %1d Data ",rx_id, rx_len);
			}
			else
			{
				printf("Standard ID : 0x%x DLC : %1d Data", rx_id, rx_len);
			}
		}
...
uint8_t mcp25625_readStatus(int fd)
{
	...
	printf("readStatus : buf[0] = 0x%x\n",buf[0]);
	printf("readStatus : buf[1] = 0x%x\n",buf[1]);
	buf[0] = buf[0] & 0x03;
	return buf[0];	
}
...
void mcp25625_read_buf(int fd, const uint8_t mcp_addr, uint8_t *ext, uint16_t *id, uint8_t nm)
{
	...
	*id = (rxbuf[MCP_SIDH] << 3) + (rxbuf[MCP_SIDL] >> 5);
	...
	/* Read EXT ID */
	if( (rxbuf[MCP_SIDL] & MCP_RXB_EXIDE_M) == MCP_RXB_EXIDE_M)
	{
		*ext = 1;
