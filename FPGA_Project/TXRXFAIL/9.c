...
{
	SPI_RX RX;
	SPI_TX TX = {0x0, 0x2, 1, 0x0, 0x8, {0,},0};
	
	uint8_t rx_status=0;
	//test
	uint32_t lidar,input;
	lidar = 912;
	input = 429;
	uint8_t i;
	//pmod variable
	...
	for(;;)
	{
		...
		if((RX.ReceiveStatus & MCP_STAT_RX0IF) == MCP_STAT_RX0IF)
		{
			...
			if(rx_status = readMsgBuf(fd_spi, &RX.id, &RX.dlc, RX.Buf, MCP_STAT_RX0IF))
			{
				printf("rx_status=%d\n",rx_status);
				for(i=0;i<7;i++)
					printf("RX.Buf[%d] = 0x%x\n",i,RX.Buf[i]);
			}
			else
			{
			}
	...
	usleep(1000);
			...
			if( (RX.Buf[8] == 0x46)  )
				{
					printf(" Receive 'F'\n");
					TX.Buf[0] = (uint8_t)(lidar & 0x0FF);
					TX.Buf[1] = (uint8_t)( (lidar >> 8) & 0x03 );
					TX.Buf[6] = (uint8_t)(input & 0x0FF);
					TX.Buf[7] = (uint8_t)( (input >> 8) & 0x03 );	
...
void mcp25625_setMsg(int fd, unsigned char *rts, unsigned char * txbuf, const uint8_t mcp_addr)
{
	...
	canid = (uint16_t)(myCanMsg.canEID & 0x0FFFF);

/* Extended ID */
	buf[MCP_EID0] = (uint8_t)(canid & 0xFF);
	buf[MCP_EID8] = (uint8_t)(canid >> 8);
	canid = (uint16_t)(myCanMsg.canEID >> 16);
