...
{
    SPI2BRG = GetPeripheralClock() / (2 * spiFreq) - 1;
    // configures SPI2
...
void SPIJA_ConfigurePins()
{
    ...
    pinMode(_SPI_SS_PIN, OUTPUT);     // RC2
    pinMode(_SPI_MOSI_PIN, OUTPUT);   // RC1
    pinMode(_SPI_SCK_PIN, OUTPUT);    // RG6
    // Configure SPIJA signals as digital inputs.
    pinMode(_SPI_MISO_PIN, INPUT);    // RC4
    
    // configure remapable pins
    RPC1R = 0x06;   // RPC1R = 0110 = SDO2 - MOSI
    SDI2R = 0x0A;   // SDI2R = 0001 = RPC4 - MISO
    
//    ansel_SPIJA_SCK = 0;  // Set ANASEL OFF ??
    
    PMCONbits.ON = 0;       // Parallel Master Port Off ???
//    RPC1R = 0;            // Disable MOSI ???
    CM1CONbits.ON = 0;      // Analog Comparitor 1 OFF ???
}
...
{
    int i;
    digitalWrite(_SPI_SS_PIN, 0); // Activate SS
    for(i = 0; i< bytesNumber; i++)
    {
        pbRdData[i] = SPIJA_RawTransferByte(pbWrData[i]);
    }
    digitalWrite(_SPI_SS_PIN, 1); // Deactivate SS
}
