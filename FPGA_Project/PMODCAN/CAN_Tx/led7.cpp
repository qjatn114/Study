...
/** Descriptive File Name: Led7.c - driver code for a 4-digit 7 segment LED
  ...
  @Description
    ...
    Uses Core Timer interrupt.  See:
http://chipkit.net/started/learn-basics/chipkit-compatible-libraries/core-timer-service-overview/
 */
/* ************************************************************************** */
...
volatile int16_t led_value = 0;
volatile int16_t led_flag = 0;
...
void seg7Init(void)
{
    ...
    DIGITS_OFF();
    clr_dsp();
    attachCoreTimerService(SevenSegLED); 
}
...
uint32_t SevenSegLED(uint32_t segDelay) 
{
...
static int ms_cntr = 10;
    ...
    return ( segDelay + CORE_TICK_RATE/100);
