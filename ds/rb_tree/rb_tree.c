...
void rb_tree_preoder_print(rb_tree *tree, rb_node *x)
{
	rb_node *nil = tree->nil;
	...
	if(x != tree->nil)
	{
		...
		rb_tree_preoder_print(tree,x->left);
		rb_tree_preoder_print(tree,x->right);
	}
}

void rb_tree_print(rb_tree *tree)
{
	rb_tree_preoder_print(tree,tree->root->left);
}
